...
class CFRubricCriterion(Base):
    ...
    __table_args__ = (
        UniqueConstraint("identifier", name="uq_cf_rubric_criteria_identifier"),
        Index("ix_cf_rubric_criteria_rubric", "cf_rubric_id"),
        Index("ix_cf_rubric_criteria_identifier", "identifier"),
    )

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    cf_rubric_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("cf_rubrics.id", ondelete="CASCADE"), nullable=False)
    identifier: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    uri: Mapped[str] = mapped_column(String, nullable=False)
    cf_item_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("cf_items.id"))
