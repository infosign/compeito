...
async def get_cf_item(
    ...
    content = dump_single(item, mode, compat_wrapper="CFItem")
...
async def get_cf_item_associations(
    ...
    content = {
        # CFAssociationSetDType: this wrapper IS the official shape, so it stays
        # in both modes (unlike the single-resource wrappers).
        "CFItem": dump_model(item, mode),
        "CFAssociations": [dump_model(a, mode) for a in assocs],
    }
    return JSONResponse(content=content, headers={"Cache-Control": CACHE_CONTROL})
