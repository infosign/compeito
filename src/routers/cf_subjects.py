...
@router.get("/{tenant}/ims/case/v1p1/CFSubjects")
async def list_cf_subjects(
    ...
    content = {"CFSubjects": [i.model_dump(by_alias=True) for i in items]}
    return JSONResponse(content=content, headers={"Cache-Control": CACHE_CONTROL})


@router.get("/{tenant}/ims/case/v1p1/CFSubjects/{id}")
async def get_cf_subject(
    ...
    objs = await case_query_service.get_cf_subject_set(session, tenant_obj.id, obj_uuid)
    if objs is None:
        raise ResourceNotFoundError(f"CFSubject not found: '{id}'")
    # CFSubjectSetDType: requested subject first, descendants by hierarchyCode follow
    content = {"CFSubjects": [o.model_dump(by_alias=True) for o in objs]}
    return JSONResponse(content=content, headers={"Cache-Control": CACHE_CONTROL})
