...
def _make_is_child_of(
    doc: CFDocument, origin_ident: uuid.UUID, dest_ident: uuid.UUID | str,
    *, seq: int | None = None,
) -> CFAssociation:
...
class TestGetChildren:
    async def test_returns_children(
        ...
        db_session.add(_make_is_child_of(
            sample_document, child.identifier, sample_document.identifier, seq=1,
        ))
        await db_session.flush()

        nodes = await tree_service.get_children(
            db_session, sample_document.id, str(sample_document.identifier),
        )
        assert len(nodes) == 1
    ...
    async def test_sorted_by_seq_hcs_ident(
        ...
        for i in [i1, i2, i3]:
            db_session.add(i)
            db_session.add(_make_is_child_of(
                sample_document, i.identifier, sample_document.identifier, seq=1,
            ))
        await db_session.flush()
    ...
    async def test_seq_null_last(
        ...
        db_session.add(_make_is_child_of(
            sample_document, i1.identifier, sample_document.identifier, seq=1,
        ))
        db_session.add(_make_is_child_of(
            sample_document, i2.identifier, sample_document.identifier, seq=None,
        ))
        await db_session.flush()
    ...
    async def test_has_children_flag(
        ...
        db_session.add(_make_is_child_of(
            sample_document, parent.identifier, sample_document.identifier, seq=1,
        ))
        db_session.add(_make_is_child_of(
            sample_document, child.identifier, parent.identifier, seq=1,
        ))
        await db_session.flush()
...
class TestGetOrphans:
    async def test_orphan_items(
        ...
        db_session.add(_make_is_child_of(
            sample_document, regular.identifier, sample_document.identifier, seq=1,
        ))
        await db_session.flush()
...
class TestBuildSSRTree:
    async def test_depth_0_expanded_depth_1_shown(
        ...
        db_session.add(_make_is_child_of(
            sample_document, root.identifier, sample_document.identifier, seq=1,
        ))
        db_session.add(_make_is_child_of(
            sample_document, child.identifier, root.identifier, seq=1,
        ))
        await db_session.flush()
    ...
    async def test_selected_item_returned(
        ...
        db_session.add(item)
        db_session.add(_make_is_child_of(
            sample_document, item.identifier, sample_document.identifier, seq=1,
        ))
        await db_session.flush()

        _, _, sel = await tree_service.build_ssr_tree(
            db_session, sample_document, item.identifier,
        )
    ...
    async def test_deep_link_expands_ancestors(
        ...
        db_session.add(_make_is_child_of(
            sample_document, mid.identifier, root.identifier, seq=1,
        ))
        db_session.add(_make_is_child_of(
            sample_document, deep.identifier, mid.identifier, seq=1,
        ))
        await db_session.flush()

        roots, _, sel = await tree_service.build_ssr_tree(
            db_session, sample_document, deep.identifier,
        )
        assert sel is not None
...
class TestGetItemForDetail:
    async def test_returns_item(
        ...
        result = await tree_service.get_item_for_detail(
            db_session, sample_document.id, item.identifier,
        )
...
class TestTreeViewPage:
    ...
    async def test_html_title(
        ...
        assert f"<title>{sample_document.title} - {tenant.name} - CASE Server</title>" in resp.text
...
class TestChildrenFragment:
    async def test_returns_children(
        ...
        resp = await db_client.get(
            f"/{tenant.id}/cftree/doc/{sample_document.identifier}"
            f"/children/{sample_document.identifier}",
        )
        assert resp.status_code == 200
    ...
    async def test_invalid_tenant_400(self, db_client):
        resp = await db_client.get(
            "/bad/cftree/doc/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"
    ...
    async def test_invalid_doc_400(
        ...
        resp = await db_client.get(
            f"/{tenant.id}/cftree/doc/bad/children/"
            "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb",
        )
        assert resp.status_code == 400
    ...
    async def test_nonexistent_item_returns_empty(
        ...
        resp = await db_client.get(
            f"/{tenant.id}/cftree/doc/{sample_document.identifier}"
            "/children/99999999-9999-9999-9999-999999999999",
        )
...
class TestDetailFragment:
    ...
    async def test_missing_item_404(
        ...
        resp = await db_client.get(
            f"/{tenant.id}/cftree/doc/{sample_document.identifier}"
            "/detail/99999999-9999-9999-9999-999999999999",
        )
        assert resp.status_code == 404
