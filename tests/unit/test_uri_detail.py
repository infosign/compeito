...
class TestFindResource:
    async def test_finds_cf_item(
        ...
        ident = uuid.uuid4()
        item = CFItem(
            tenant_id=tenant.id, cf_document_id=sample_document.id,
            identifier=ident, uri="u", full_statement="stmt",
            last_change_date_time=NOW, depth=0,
        )
        db_session.add(item)
        await db_session.flush()

        result = await uri_service.find_resource_by_identifier(
            db_session, tenant.id, ident,
        )
    ...
    async def test_finds_cf_document(
        ...
        result = await uri_service.find_resource_by_identifier(
            db_session, tenant.id, sample_document.identifier,
        )
        assert result is not None
    ...
    async def test_finds_cf_association(
        ...
        assoc = CFAssociation(
            tenant_id=tenant.id, cf_document_id=sample_document.id,
            identifier=ident, uri="u", association_type="isChildOf",
            origin_node_uri="ou", origin_node_identifier=str(uuid.uuid4()),
            destination_node_uri="du", destination_node_identifier=str(uuid.uuid4()),
    ...
    async def test_finds_lookup_item_type(
        ...
        ident = uuid.uuid4()
        it = CFItemType(
            tenant_id=tenant.id, identifier=ident, uri="u",
            title="Standard", last_change_date_time=NOW,
        )
        db_session.add(it)
        await db_session.flush()
    ...
    async def test_finds_lookup_subject(
        ...
        ident = uuid.uuid4()
        db_session.add(CFSubject(
            tenant_id=tenant.id, identifier=ident, uri="u",
            title="Math", last_change_date_time=NOW,
        ))
        await db_session.flush()
    ...
    async def test_finds_lookup_concept(
        ...
        ident = uuid.uuid4()
        db_session.add(CFConcept(
            tenant_id=tenant.id, identifier=ident, uri="u",
            title="Algebra", last_change_date_time=NOW,
        ))
        await db_session.flush()
    ...
    async def test_finds_lookup_license(
        ...
        ident = uuid.uuid4()
        db_session.add(CFLicense(
            tenant_id=tenant.id, identifier=ident, uri="u",
            title="MIT", last_change_date_time=NOW,
        ))
        await db_session.flush()
    ...
    async def test_finds_lookup_grouping(
        ...
        ident = uuid.uuid4()
        db_session.add(CFAssociationGrouping(
            tenant_id=tenant.id, identifier=ident, uri="u",
            title="Group A", last_change_date_time=NOW,
        ))
        await db_session.flush()
    ...
    async def test_item_priority_over_document(
        ...
        item = CFItem(
            tenant_id=tenant.id, cf_document_id=sample_document.id,
            identifier=shared_ident, uri="u", full_statement="item",
            last_change_date_time=NOW, depth=0,
        )
        db_session.add(item)
        await db_session.flush()
        ...
        result = await uri_service.find_resource_by_identifier(
            db_session, tenant.id, shared_ident,
        )
        assert result.resource_type == "CFItem"
...
class TestUriDetailPage:
    async def test_cf_item_page(
        ...
        item = CFItem(
            tenant_id=tenant.id, cf_document_id=sample_document.id,
            identifier=ident, uri="https://example.com/item",
    ...
    async def test_cf_association_page(
        ...
        assoc = CFAssociation(
            tenant_id=tenant.id, cf_document_id=sample_document.id,
            identifier=ident, uri="https://example.com/assoc",
    ...
    async def test_html_title_item(
        ...
        item = CFItem(
            tenant_id=tenant.id, cf_document_id=sample_document.id,
            identifier=uuid.uuid4(), uri="u", full_statement=stmt,
            last_change_date_time=NOW, depth=0,
        )
        db_session.add(item)
        await db_session.flush()
...
class TestUriSecurityUrls:
    ...
    async def test_javascript_url_is_text(
        ...
        item = CFItem(
            tenant_id=tenant.id, cf_document_id=sample_document.id,
            identifier=uuid.uuid4(), uri="javascript:alert(1)",
            full_statement="test", last_change_date_time=NOW, depth=0,
        )
        db_session.add(item)
        await db_session.flush()
