...
- **CFPckg* schemas inside CFPackage**: CASE v1.1 defines CFPckg-specific shapes inside CFPackage. compeito's implementation keeps the structural distinction but **emits the LinkURI fields anyway** (`CFPackageURI` / `CFDocumentURI`) for round-trip parity with OpenCASE (see [round_trip_status.md](../dev/round_trip_status.md) cat B / D). The CASE v1.1 spec permits this — the fields are redundant inside a package context but echoing them aids clients that walk individual resources.
  - `CFPckgDocument`: standalone `CFDocument`; **includes** `CFPackageURI`.
  - `CFPckgItem`: standalone `CFItem`; **includes** `CFDocumentURI`.
  - `CFPckgAssociation`: standalone `CFAssociation` minus `CFDocumentURI` (unchanged from earlier — round-trip parity already satisfied without it).
...
- **CFPackage 内のリソーススキーマ（CFPckg* 型）**: CASE v1.1 では CFPackage 内のリソースはスタンドアロン型とは異なる CFPckg* 型を定義する。compeito の実装は構造上の区別は維持しつつ、**OpenCASE round-trip の整合性を取るため `CFPackageURI` / `CFDocumentURI` の LinkURI 系は emit する**（詳細は [round_trip_status.md](../dev/round_trip_status.md) の cat B / D。CASE v1.1 仕様上、これらのフィールドはパッケージ文脈では冗長だが echo は許可されており、個別リソースを舐めるクライアントの助けにもなる）:
  - `CFPckgDocument`: スタンドアロン `CFDocument` を踏襲し、`CFPackageURI` を**含める**
  - `CFPckgItem`: スタンドアロン `CFItem` を踏襲し、`CFDocumentURI` を**含める**
  - `CFPckgAssociation`: スタンドアロン `CFAssociation` から `CFDocumentURI` を**除外**（CFAssociation 側は round-trip 整合性が既に取れており、変更不要）
