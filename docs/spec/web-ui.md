...
| GET /{tenant-uuid}/cftree/doc/{doc-uuid} | Tree view. The **full tree is server-rendered** with native `<details>` expand/collapse (no JS). `?item={uuid}` opens the path to that item and SSRs its detail in the right pane. |
...
| GET /{tenant-uuid}/cftree/doc/{doc-uuid} | ツリービュー。**ツリー全体を SSR** し、ネイティブ `<details>` で開閉（JS不要）。`?item={uuid}` で当該アイテムまで開いて右ペインに詳細を SSR |
