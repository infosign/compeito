...
- **右ペイン**: 選択アイテムの詳細を HTMX `hx-get`（`GET /{tenant}/cftree/doc/{doc-uuid}/detail/{item-uuid}`）で差し替え表示。冗長な「ツリーで表示」リンクは**ペインでは非表示**（`in_pane`）— 単独 `/uri/` ページでのみ出す。
  - **ペイン内ナビ**: 「関連」リンクの遷移先が**同一ドキュメント内の CFItem** の場合、ツリー内で完結する — ペインを差し替え（`hx-get` detail）、パス形式 URL を push、`selectTreeNode(id)`（JS）で対象の祖先 `<details>` を開いてハイライト＋スクロール。同一ドキュメントの item でない遷移先（別ドキュメント、ルーブリック基準・定義などツリー外のリソース）は当面 `/uri/` 全画面リンクのまま（別ドキュメントのツリー切替は後段）。同一ドキュメント判定と並び順はサーバー側 `tree_service.sort_related_by_tree_order` が計算し、各関連グループを**ツリーと同じキー**（`_child_sort_key`：遷移先の isChildOf `sequence_number` 昇順・NULL 最後 → `human_coding_scheme` 自然順・NULL 最後 → identifier。同一doc を先頭）で並べ替えるので、`sequence_number` が hcs 順と異なる場合も含めツリーと同じ順序で読める。
