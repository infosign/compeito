...
identifier: UUID NOT NULL               -- parent-scoped uniqueness (see UNIQUE below)
uri: VARCHAR NOT NULL
cf_item_id: UUID FK(cf_item.id) NULLABLE  -- CASE v1.1 CFItemURI; FK to the related CFItem
cf_item_uri_source: TEXT                 -- source CFItemURI.uri preserved verbatim for OpenCASE round-trip; rebuilt into a LinkURI at emit time
...
last_change_date_time: TIMESTAMP NOT NULL
UNIQUE(cf_rubric_id, identifier)    -- parent-scoped (#187); the same UUID can be imported under different rubrics / tenants
...
position: INTEGER
last_change_date_time: TIMESTAMP NOT NULL
UNIQUE(cf_rubric_criterion_id, identifier)  -- parent-scoped (#187); same rationale as cf_rubric_criterion
...
（教育課程の基準等）をインポートするユースケースがあるため、テナント単位のユニークに緩和する。
...
### uuid横断検索用（/uri/{uuid}）
`identifier` はテナントスコープの各テーブルで `UNIQUE(tenant_id, identifier)` の複合ユニーク制約を持つ（ルーブリック子テーブル `cf_rubric_criterion` / `cf_rubric_criterion_level` は親スコープ。各定義参照）。
...
id: UUID PK
cf_rubric_id: UUID FK(cf_rubric.id) NOT NULL
identifier: UUID NOT NULL               -- 親スコープで一意（下記 UNIQUE 参照）
uri: VARCHAR NOT NULL
cf_item_id: UUID FK(cf_item.id) NULLABLE  -- CASE v1.1 CFItemURI。関連する CFItem への FK 参照
cf_item_uri_source: TEXT                 -- source の CFItemURI.uri を verbatim 保持（OpenCASE round-trip 用）。emit 時に LinkURI に再構築
...
last_change_date_time: TIMESTAMP NOT NULL
UNIQUE(cf_rubric_id, identifier)    -- 親スコープ（#187）。同一 UUID を別ルーブリック / 別テナントでインポート可能
INDEX(cf_rubric_id), INDEX(identifier)
```
...
position: INTEGER
last_change_date_time: TIMESTAMP NOT NULL
UNIQUE(cf_rubric_criterion_id, identifier)  -- 親スコープ（#187）。cf_rubric_criterion と同じ理由
INDEX(cf_rubric_criterion_id), INDEX(identifier)
```
