...
- `CFDocumentURI`, `notes` 等の非保存フィールドは無視する（`notes` は CASE v1.1 新規の任意フィールドだが Phase 1 では省略。db-schema.md 参照）
...
- `notes`（CFDocument / CFItem / CFAssociation）: CASE v1.1 の任意フィールドだが DB スキーマに含めていない。外部インポート時に値が失われる
- `alternativeLabel`（CFItem）: 同上
- `extensions`（全リソース共通、v1.1 新規）: 拡張データ用の任意フィールドだが DB スキーマに含めていない。外部インポート時に値が失われる
...
- テナント内全体で `identifier` 一致検索
- 一致する場合は更新（非 null フィールドのみ上書き）
- 一致しない場合は新規作成
- CFRubricCriterion / CFRubricCriterionLevel も同様に `identifier` 一致で upsert
...
- URL に含まれない場合でも、ルートに `CFPackage` ラッパーがなく `CFDocument.caseVersion` が存在しなければ v1.0 と判定
...
- **Criterion**: 親 Rubric を `RubricIdentifier` または位置コンテキスト（直前の Rubric 行）から解決。`identifier` で既存 CFRubricCriterion を検索し upsert。`CFItemIdentifier` が指定されている場合は同一テナント内の CFItem から FK 解決（見つからない場合は NULL + 警告）
- **Level**: 親 Criterion を `CriterionIdentifier` または位置コンテキスト（直前の Criterion 行）から解決。`identifier` で既存 CFRubricCriterionLevel を検索し upsert
