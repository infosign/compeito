...
- **Exception — lifecycle dates.** For `statusStartDate` / `statusEndDate` (CFDocument and CFItem), an explicit `null` **clears** the stored value; only a missing key preserves it. This is what lets a retired item be revived: the producer sends `"statusEndDate": null` and the tombstone is lifted. Exporters that do not manage these fields omit the key entirely, so they still cannot wipe them. Applies to CFPackage (CASE JSON) import only — CSV / xlsx keep the "empty cell → preserve" rule.
...
- `statusEndDate` → `status_end_date` (same rule as `statusStartDate`). On update, an explicit `null` clears both fields (see "Exception — lifecycle dates" above).
...
- `statusEndDate` → `status_end_date` (same as `statusStartDate`). On update, an explicit `null` clears both fields (see "Exception — lifecycle dates" above).
...
- **例外：ライフサイクル日付**。`statusStartDate` / `statusEndDate`（CFDocument および CFItem）は、明示的な `null` で既存値を**クリア**する。保持されるのはキー自体が未存在の場合のみ。廃止された項目の取り消し（墓標の解除）を可能にするためで、生成側は `"statusEndDate": null` を送る。この2フィールドを管理していないエクスポーターはキーごと出力しないため、誤って消すことはない。CFPackage（CASE JSON）インポートのみに適用され、CSV / xlsx は「空セル → 保持」の規則のままとする
...
- `statusEndDate` → `status_end_date`（`statusStartDate` と同一ルール）。更新時、明示的な `null` は両フィールドをクリアする（上記「例外：ライフサイクル日付」を参照）
