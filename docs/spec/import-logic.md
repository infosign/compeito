...
- `uri` → `uri`（FR-7.2: source URI を **新規・更新の両方で** verbatim 保持。CFItem 側と同じ `_resolve_uri()` で処理）。**自インスタンスを指しているのにテナントが違う URI も verbatim で保存し、警告を出す**（テナントセグメントが slug の場合と、別テナントを指している場合）。slug は改名可能な UI 上の別名で CASE 応答には出ないため、改名した時点で解決しなくなる。誤ったテナント向けに作られたパッケージは全 URI が同時に誤るので、警告は種類ごとに集約する。別ホストの URI は通常の外部参照として扱い、警告しない
