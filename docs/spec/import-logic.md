...
**UUID case sensitivity:** UUID identifiers are compared **case-insensitively** (upsert matching, `parentIdentifier` resolution, `/uri/{uuid}` lookup, etc.). PostgreSQL's UUID type is case-insensitive — `D86774F2-...` equals `d86774f2-...`. On create we normalize to lowercase when storing. UUIDs from external imports are stored as-is, and the case-insensitive rule still applies on comparison. **Exception — association node identifiers** (`origin_node_identifier` / `destination_node_identifier`) are **String** columns (not UUID type), so they are explicitly lowercased on import when they are UUIDs; otherwise an uppercase external UUID would never string-match a CFItem's lowercase identifier in related / cross-document hierarchy lookups. Non-UUID external node refs are kept as-is.
...
- `statusStartDate` → when non-empty, validate `YYYY-MM-DD`. On invalid format or value (e.g., `2025-13-45`), emit a warning ("Row N: Invalid statusStartDate 'xxx', ignored; existing value kept on update") and treat the field as **absent** — on update the stored date is preserved, on create it stays NULL. (Unlike the other fields, a malformed lifecycle date must not wipe a retirement date: the CSV / xlsx path has no explicit clearing mechanism. Same policy as the CASE JSON path.)
...
- `#status_start_date` / `#status_end_date` → when non-empty, validate `YYYY-MM-DD`. Invalid format or value → warning ("Invalid #status_start_date 'xxx', ignored; existing value kept on update"); treat as **absent** (the stored date is preserved on update, NULL on create) — same lifecycle-date policy as the item columns.
...
- **Existing associations on upsert**: the rebuild is **document-level** (same as `isChildOf`). Each association type **whose column is present in the header** is deleted across the whole document and rebuilt from the CSV's rows (the deleted count is reported as `existing_associations_deleted`). A type whose column is absent is left untouched — so associations imported via CASE JSON survive a CSV round-trip that does not mention them. **Caveat**: because the rebuild is document-level, a *partial* CSV (one omitting some items) also drops the present-type associations of the omitted items; re-export the full document before editing (same round-trip caveat as `isChildOf`). An empty cell — or an item with no row — therefore ends up with no links of that type. As with `isChildOf`, this runs only when the CSV has ≥1 data row.
...
- `title` → `title` (stored verbatim, like `fullStatement`; a whitespace-only title is rejected by structure validation).
...
- `uri` → `uri` (FR-7.2: source URI preserved verbatim on **both** create and update — `_resolve_uri()` in `case_import_service.py`. Falls back to `{BASE_URL}/{tenant}/uri/{identifier}` only when the source omits `uri`. The same rule applies to CFDocument / CFAssociation / CFRubric* / lookup resources). **A URI that points at this instance but at the wrong tenant is still stored verbatim, with a warning**: either the tenant segment is a slug (a renameable UI alias — CASE responses never emit it, so the URI stops resolving at the next rename) or it addresses a different tenant. Warnings are aggregated by kind, since a package built against the wrong tenant gets every URI wrong at once. A URI on another host is a normal external reference and is not flagged, and neither is any other path on this host — an instance often serves an ordinary site next to compeito, so only the tenant-addressed shapes this app routes (`/{tenant}/uri/{id}`, `/{tenant}/ims/...`) are judged. Association endpoint URIs (`originNodeURI` / `destinationNodeURI`) are out of scope for now: they legitimately point at other tenants and at external frameworks.
...
**Only the second bullet is live on the CASE JSON path.** `import case` is additive by policy (see the tombstone rules above): it never deletes an association, so nothing can be lost and `CaseImportReport` carries no loss counter. A package that omits associations does not remove them — the old ones simply stay. The guard is therefore near-silent for `import case`, and a consumer whose main path is CASE JSON should know that it is guarding against copied identifiers, not against a shrinking package. Detecting *stale* structure left behind by an additive import — an association still in the document that the submitted package does not contain — is a different question from a net loss, and is tracked separately as [feature backlog](../dev/backlog.md) B10.
...
- `statusStartDate` → 非空の場合、`YYYY-MM-DD` 形式の有効な日付かを検証する。形式不正または無効な日付（例: `2025-13-45`）の場合は警告を出力し（「Row N: Invalid statusStartDate 'xxx', ignored; existing value kept on update」）、該当フィールドを**未指定**として扱う（更新時は既存の日付を保持、新規作成時は NULL。行スキップではない）。他のフィールドと違い、壊れたライフサイクル日付で廃止日を消してはならない（CSV / xlsx 経路にはクリア手段が無い）。CASE JSON 経路と同じ方針
...
- `#status_start_date` / `#status_end_date` → 非空の場合、`YYYY-MM-DD` 形式の有効な日付かを検証する。形式不正または無効な日付の場合は警告を出力し（「Invalid #status_start_date 'xxx', ignored; existing value kept on update」）、**未指定**として扱う（更新時は既存の日付を保持、新規作成時は NULL）。アイテム列と同じライフサイクル日付の方針
...
- **upsert時の既存Association処理**: 再構築は **ドキュメント単位**（`isChildOf` と同一）。ヘッダーに**列が存在する** association タイプは、そのドキュメント全体で削除してから CSV の行をもとに再生成する（削除件数は `existing_associations_deleted` として報告）。列が存在しないタイプは温存する（CASE JSON 経由でインポートした Association が、それに言及しない CSV の往復で消えないようにするため）。**注意**: 再構築がドキュメント単位のため、一部のアイテムを省いた**部分的な CSV** では、省かれたアイテムの当該タイプの Association も削除される。編集前にドキュメント全体を再エクスポートすること（`isChildOf` と同じ round-trip 制約）。したがって空セル（または行自体がないアイテム）は、結果として当該タイプのリンクを持たなくなる。`isChildOf` と同様、データ行が 1 件以上ある場合にのみ実行する
...
- `uri` → `uri`（FR-7.2: source URI を **新規・更新の両方で** verbatim 保持。CFItem 側と同じ `_resolve_uri()` で処理）。**自インスタンスを指しているのにテナントが違う URI も verbatim で保存し、警告を出す**（テナントセグメントが slug の場合と、別テナントを指している場合）。slug は改名可能な UI 上の別名で CASE 応答には出ないため、改名した時点で解決しなくなる。誤ったテナント向けに作られたパッケージは全 URI が同時に誤るので、警告は種類ごとに集約する。別ホストの URI は通常の外部参照として扱い、警告しない。同一ホスト上でも、このアプリがルーティングする形（`/{tenant}/uri/{id}`、`/{tenant}/ims/...`）以外のパスは判定しない（compeito の隣で通常の Web サイトを配信している構成があり、そこを指す `uri` は正当なため）。関連の endpoint URI（`originNodeURI` / `destinationNodeURI`）は当面対象外とする（別テナントや外部フレームワークを指すのが正当なため）
- `title` → `title`（`fullStatement` と同じく原文のまま保存する。空白のみのタイトルは構造検証で弾かれる）
...
**CASE JSON 経路で生きているのは2つめだけである。** `import case` は方針として additive であり（上の墓標の規則を参照）、関連を削除しない。したがって失われるものが無く、`CaseImportReport` に消失のカウンタは持たせていない。関連を省いたパッケージを取り込んでも消えるのではなく、古いものがそのまま残る。つまり `import case` ではガードはほとんど沈黙し、見ているのは identifier のコピーであってパッケージの縮小ではない。additive な取り込みが残した**古い関連**の検知 — 取り込み後の文書に、送られたパッケージに無い関連が残っている状態 — は正味の損失とは別の問いであり、[機能バックログ](../dev/backlog.md) B10 として分けて管理する。
