...
- `cf_item.concept_keywords_uri`: 各 cf_concept レコードの `{title, identifier, uri}` から LinkURI オブジェクト配列を構築（例: `[{"title":"分析","identifier":"<cf_concept.identifier>","uri":"<cf_concept.uri>"}]`）

**更新時の連動ルール:**
- CSVの `conceptKeywords` に値がある → `concept_keywords` と `concept_keywords_uri` の両方を新しい値で再構築する
- CSVの `conceptKeywords` が空セル → `concept_keywords` と `concept_keywords_uri` の両方とも既存値を保持する
...
Import Result:
  ...
  Concepts:     12 created, 0 updated, 0 existing, 0 skipped
...
5. CFItems を一括保存
   ...
   - `educationLevel`, `conceptKeywords`, `conceptKeywordsURI` → 外部データの値をそのまま JSONB として保存する（FK解決なし）
...
- `conceptKeywordsURI` → `concept_keywords_uri`（JSONB。外部データをそのまま保存）
...
- **`concept_keywords_uri` / `subject_uri` の round-trip 制約**: これらの LinkURI 配列フィールドは CSV に出力されない（`conceptKeywords` はテキスト値のみ、`#subject` は subject 名のみ出力）。再インポート時にローカル lookup テーブルから URI を再構築するため、外部 CASE ソースインポート由来の外部 URI はローカル URI に置き換わる。同一テナント内の更新では lookup レコードの identifier が一致するため URI は維持されるが、新規テナントへのインポートでは新しい identifier・URI が採番される
