...
- **strict/compat 出力モード（2026-08）**: `?strict=1` / `?compat=1` を全 CASE GET エンドポイントで受け付ける。wrapper 除去（C1）・`exclude_none`（C16）・パッケージ内 URI 除去（C2）・`caseVersion="1.1"` 宣言（C8 出力側）を適用する。既定は `settings.case_output_default`（現状 `compat`）で、既定反転はメジャーバージョンイベントとして別途実施する。設計: [designs/strict-output.md](./designs/strict-output.md)
...
|---|------|------|:--:|------|
| ~~C16~~ **対応済み** | **optional フィールドの null 出力**（exclude_none 未使用） | （〜2026-08）全 DType が optional フィールドを `null` で emit しており、公式スキーマは nullable を定義しないため**型違反**だった。**当時最大の schema-invalid 源**（2026-07 外部レビュー N1） | — | **実装済み（2026-08）**: strict 出力で `exclude_none=True` を全 CASE GET エンドポイントに適用。`caseVersion` の `"1.1"` 宣言（C8 出力側）と docstring / api-spec の文言修正（N7）も同時に完了。既定（compat）の出力は不変 |
| C18 | **discovery で配信する OpenAPI が未 localize** | §2.5 は "A Service Provider MUST provide a **localized version** of the OpenAPI file" と規定し、OpenAPI 側も `servers.variables.hostName` に "MUST be set to the actual service provider"、`Contact` も実サービスのものへ変更すべきとしている。現状は公式配布物をそのまま返しており、エンドポイントの存在は満たすが内容が未 localize | P1 | **単純な差し替えでは解決しない。以下を設計課題として詰める必要がある。** (a) 公式 `basePath` は `enum: ["/ims/case/v1p1"]` 固定だが、実 API はテナント配下 （`/{tenant}/ims/case/v1p1`）。enum を守ると実 URL と一致せず、実 URL に合わせると enum 違反になる。(b) discovery 自体はテナント非依存なので、どのテナントの API を記述するか決まらない。1 deployment = 1 tenant とするか、テナント固有 discovery を拡張として設けるか、標準パスを特定テナントにマップするかの選択が必要。(c) ホスト名は未検証の `Host` ヘッダーではなく、設定された公開 URL から生成する。(d) Binding は `Contact` を実サービスのものに変更すべきとしているため、変更の可否ではなく**どの role アドレス / URL を正式な contact とするか**が課題（個人アドレスではなく運用継続可能な role account）。(e) ライセンス面: 同梱ファイルを無改変で保持しても、変更した JSON を公開配信すればその応答は改変された表現であり、「実行時生成だから派生物ではない」とは言い切れない。§2.5 が localize を MUST とし OpenAPI 側も host 変更を明記しているため実装上必要な範囲の localization には強い根拠があるが、1EdTech への照会に含めて確認する。(f) 実装時は [designs/openapi-schema-validation-tests.md](./designs/openapi-schema-validation-tests.md) の discovery ドリフトガード（応答 == SPEC）を、localize 後の期待値との比較か localize 対象フィールドの除外に変更する |
...
| ~~C1~~ **対応済み** | **単一リソースの wrapper** | `{"CFDocument": {...}}` で包む（OpenSALT 流）。公式は flat DType（root に直接） | — | **実装済み（2026-08）**: `?strict=1` で単一リソース6ルートの wrapper を外す。Set 型・`CFItemAssociations`・list 系は公式形が wrapper なので据え置き |
| ~~C2~~ **対応済み** | **パッケージ内 URI の既定出力** | 既定で `CFPackage.CFDocument.CFPackageURI` / `CFItems[].CFDocumentURI` を出す（OpenCASE/OpenSALT 互換）。公式 `CFPckg*DType` は `additionalProperties:false` で非許容。`?strict=1` で除去可 | — | **実装済み（2026-08）**: `?strict=1` で除去。既定を strict 側へ反転するのは第2段（`case_output_default` の変更）で、メジャーバージョンイベントとして別途実施する |
| C3 | **required だが nullable な項目**（出力層は対応済み） | `creator`、lookup の `hierarchyCode`（CFItemType/CFConcept/CFSubject）、CFItemType の `description`、`licenseText`、`LinkGenURIDType.title` を null 許容（寛容 import 優先） | P1 | **決定（2026-08、案 (a)）**: strict では `exclude_none` によりキー自体を省略する（型違反は解消し、違反は「required キー欠落」に変わる）。捏造もプレースホルダ合成もしない。残ギャップは**データ起因**であり、import 時の validation report（B6）で検出して運用で補完する |
