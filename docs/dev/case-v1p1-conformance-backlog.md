...
| C18 | **discovery で配信する OpenAPI が未 localize** | §2.5 は "A Service Provider MUST provide a **localized version** of the OpenAPI file" と規定し、OpenAPI 側も `servers.variables.hostName` に "MUST be set to the actual service provider"、`Contact` も実サービスのものへ変更すべきとしている。現状は公式配布物をそのまま返しており、エンドポイントの存在は満たすが内容が未 localize | P1 | **単純な差し替えでは解決しない。以下を設計課題として詰める必要がある。** (a) 公式 `basePath` は `enum: ["/ims/case/v1p1"]` 固定だが、実 API はテナント配下 （`/{tenant}/ims/case/v1p1`）。enum を守ると実 URL と一致せず、実 URL に合わせると enum 違反になる。(b) discovery 自体はテナント非依存なので、どのテナントの API を記述するか決まらない。1 deployment = 1 tenant とするか、テナント固有 discovery を拡張として設けるか、標準パスを特定テナントにマップするかの選択が必要。(c) ホスト名は未検証の `Host` ヘッダーではなく、設定された公開 URL から生成する。(d) `info.contact` を実サービスのものに変えるかも検討する。(e) ライセンス面: 同梱ファイルを無改変で保持しても、変更した JSON を公開配信すればその応答は改変された表現であり、「実行時生成だから派生物ではない」とは言い切れない。§2.5 が localize を MUST とし OpenAPI 側も host 変更を明記しているため実装上必要な範囲の localization には強い根拠があるが、1EdTech への照会に含めて確認する。(f) 実装時は [designs/openapi-schema-validation-tests.md](./designs/openapi-schema-validation-tests.md) の discovery ドリフトガード（応答 == SPEC）を、localize 後の期待値との比較か localize 対象フィールドの除外に変更する |
| C17 |
