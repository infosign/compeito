...
| B10 | **additive な取り込みが残す古い構造の検知** — `import case` は削除しないので、関連を省いたパッケージを取り込んでも古い関連が残る。項目が親を2本持つ等、取り込み後の構造が送られたパッケージと食い違う状態を検知したい。B6 の破壊的変更ガード（正味の損失）は CASE 経路では `itemsMoved` / `associationsMoved` しか見ておらず、この形を拾えない。**多重親は CASE として合法**（B8-4 の廃止判定も多重親の DAG を前提にしている）なので、エラーではなく報告として扱い、ゲートに使うなら opt-in とする。compeito-aws#100 で提起 | 未着手 | — |
