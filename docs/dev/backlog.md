...
| B10 | **additive な取り込みが残す古い関連の検知** — `import case` は削除しないので、関連を省いたパッケージを取り込んでも古い関連が残る。検知したいのは「**取り込み後の文書に、送られたパッケージに無い関連が残っている**」状態であり、判定はパッケージとの突き合わせ（identifier 基準）になる。当初は「項目が親を2本以上持つ」で拾うつもりだったが、実データで否定された（compeito-aws#100）。`isChildOf` の identifier だけが変わり親は同じ版では、関連が倍になっても多重親は 0 件になる。多重親は結果のひとつであって原因ではない。なお**多重親は CASE として合法**（B8-4 の廃止判定も多重親の DAG を前提にしている）なので、エラーではなく報告として扱い、ゲートに使うなら opt-in とする | 未着手 | — |
...
B8-1〜B8-7 はすべて完了している（B8-5 / B8-6 は設計への反映が成果物で、実装は B4 / B2 / B5 の実装時に行う）。
